...
def projective(img: np.ndarray, transform_matrix: np.ndarray,
               pretransform_resize_factor: int = 1) -> np.ndarray:
    """
    Loads a raw image, applies simple demosaicing and transforms the image with the
    predefined perspective projection (fit from rendered huawei jpgs).

    Args:
        path: path to file.
        cfa_pattern: list containig the pattern for demosaicing. Supports 2x2 mosaic patterns.
        transform_matrix: transformation matrix for perspective projection.
        pretransform_resize_factor: factor to scale the image down by. Useful when scaling was used
            to find a transformation matrix. Defaults to 1.
             
    Returns:
        preprocessed image.
    """
    raw = img
    res_raw = cv2.resize(raw, (raw.shape[1] // pretransform_resize_factor,
                                   raw.shape[0] // pretransform_resize_factor))

    trans_raw = cv2.warpPerspective(res_raw, transform_matrix, (3072, 4096))
    return trans_raw
